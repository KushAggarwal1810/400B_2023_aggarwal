...
class M33AnalyticOrbit:
    ...
    def __init__(self, filename):
        ...
        self.Vc = 180 # circular speed in Km/s (M31 Halo Vcirc from Hw5)
    ...
    def DynamicalFriction(self,Msat,Vc,r,v):
        ...
        """
        Method that returns the deaccelration from duynaical friction a= -0.428
         G Msat Ln(Lambda) / rmag**2
          * v/vmag        
          ...
          Max circular velcoity of the host's M31 dark matter halo in (km/s)
          
          r: 'np.ndarray'
          
          Position ovector of the satellite relative to the host (rM33- rM31) in kpc
          
          v : 'np.ndarray'
          
          Velocity vector of the satellite realtive to the host (vM33- vM31) in km/s
          ...
          DF: 'np.ndarray'
            ...
            Deacceleration vector owing to Dynamical friction (Km/s)
          
          """
        r_mag = np.sqrt(r[0]**2 + r[1]**2 + r[2]**2) #Magntitude of the position vector
        v_mag = np.sqrt(v[0]**2+  v[1]**2 + v[2]**2) #magnitude of the veclity vector
         
        bmax = r_mag # current separation between M33 and M31
        
        bmin = self.G*Msat/Vc**2 # G Msat /Vc**2
        
        Coloumb = np.log(bmax/bmin)
        
        DF= -0.428*self.G*Msat*Coloumb/r_mag**2*v/v_mag
    ...
    def M31Accel(self, r, v):
        ...
        v : 'np.ndarray'
            velcoity vector for DF calculation ( km/s)
        ...
        a = ahalo+abulge+adisk+ aDF   # Now it has deaccelration term from Dynamical friction
...
if __name__ == '__main__':
    ...
    ax.plot(M33SimOrbit['t'], M31_M33_R, 'g', lw=5, label='M31-M33 with DF')
