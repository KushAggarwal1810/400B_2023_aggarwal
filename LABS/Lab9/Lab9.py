...
class M33AnalyticOrbit:
    ...
    def __init__(self, filename, fudge):
        '''
    ...
    def DynamicalFriction(self,Msat,Vc,r,v):
        '''
        Returns the deceleration from dynamical fricton
        a = -0.428*G*Msat*ln(lambda)/rmag^2*v/vmag
        INPUTS
        ------
        Msat (float): Mass of the satellite (Msun)
        Vc (float): Max circular velocity of the host's DM halo (km/s)
        r (np array of floats): Position vector of satellite relative to host (kpc)
        v (np array of floats): Velocity vector of satellite relative to host (km/s)
        OUTPUTS
        ------
        DF (np array of floats): Deceleration vector from DF (km/s^2)
        '''
...
M33 = M33AnalyticOrbit("M33AnalyticOrbit_withDF.txt", 1.0)
M33_fudge = M33AnalyticOrbit("M33AnalyticOrbit_withDF+fudge.txt", np.e**2/10)

# Compute the orbit
M33.OrbitIntegration(0, 0.1, 10.)
M33_fudge.OrbitIntegration(0,0.1,10.)
# With DF 
...
M33OrbitFudge = np.loadtxt("M33AnalyticOrbit_withDF+Fudge.txt", dtype=orbit_type)
...
M31_M33_R_fudge, M31_M33_V_fudge = vector_mag(M33OrbitFudge)
# No DF 
...
ax.plot(M33Orbit['t'],M31_M33_R,'g',lw=5,linestyle='--',label='M31-M33 w/ DF')
ax.plot(M33OrbitFudge['t'],M31_M33_R_fudge,'p',lw=5,linestyle='-.',label='M31-M33 w/ DF+Fudge')
...
ax.plot(M33Orbit['t'], M31_M33_V, 'g', lw=5, ls='--', label='M31-M33 w/ DF')
ax.plot(M33OrbitFudge['t'], M31_M33_V_fudge, 'p', lw=5, ls='-.', label='M31-M33 w/ DF+Fudge')
